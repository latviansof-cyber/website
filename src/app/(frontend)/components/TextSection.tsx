...
export function TextSection({ id, tone = 'plain' }: { id: SectionId; tone?: Tone }) {
  ...
  return (
    <Section id={id} ariaLabel={data.title} tone={tone} className="py-24 sm:py-32 relative overflow-hidden">
      ...
      <Container className="max-w-6xl relative z-10">
        <div className="flex flex-col items-center text-center mb-16 sm:mb-20">
          <Eyebrow className="text-sunset-orange tracking-widest font-bold uppercase mb-4">{label}</Eyebrow>
          <h2
            id={`${id}-title`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className={`space-y-6 text-pretty text-lg leading-relaxed text-ink-light sm:text-xl sm:leading-[1.8] font-medium drop-shadow-sm bg-white/50 backdrop-blur-sm p-8 sm:p-10 rounded-3xl border border-white/60 shadow-lg ${id === 'about' ? '' : 'lg:order-last'}`}>
            {data.body.map((paragraph, idx) => (
              <p key={idx} className="first-letter:text-5xl first-letter:font-serif first-letter:font-bold first-letter:text-sunset-red first-letter:mr-2 first-letter:float-left first-letter:mt-1">
                {paragraph}
              </p>
            ))}
          </div>
